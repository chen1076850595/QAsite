...
import com.zhihu.QAsite.service.FeedService;
...
public class FeedController {
    ...
    private String feeds(Model model){
    ...
    private String getPullFeeds(Model model) {
        ...
        if (localUserId != 0) {
            // 关注的人
            followees = followService.getFollowees(localUserId, Entity.ENTITY_USER, Integer.MAX_VALUE);
        }
        List<Feed> feeds = feedService.getUserFeeds(Integer.MAX_VALUE, followees, 20);
        model.addAttribute("feeds", feeds);
        return "feeds";
    }
}
