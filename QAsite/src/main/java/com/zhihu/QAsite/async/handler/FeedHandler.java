...
public class FeedHandler implements EventHandler {
    ...
    private String buildFeedData(EventModel model) {
        ...
        if (model.getType() == EventType.COMMENT ||
                (model.getType() == EventType.FOLLOW  && model.getEntityType() == Entity.ENTITY_QUESTION)) {
    ...
    public void doHandler(EventModel model) {
      ...
      //  model.setActorId(1+r.nextInt(10));
        ...
        System.out.println("执行了feed");

        // 获得所有粉丝
        List<Integer> followers = followService.getFollowers(Entity.ENTITY_USER, model.getActorId(), Integer.MAX_VALUE);
        // 系统队列
        followers.add(0);
        // 给所有粉丝推事件
